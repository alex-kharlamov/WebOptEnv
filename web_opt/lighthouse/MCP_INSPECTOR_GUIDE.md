...
```bash
npx @modelcontextprotocol/inspector docker run -i --rm --network host web_opt-env:latest node /app/mcp/dist/index.js
```
...
Navigate to the root WebOptEnv directory (not web_opt):
...
```json
{
  "mcpServers": {
    "web-opt-env": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "--network", "host", "web_opt-env:latest", "node", "/app/mcp/dist/index.js"]
    }
  }
}
```
...
- **Server Dockerfile**: `/path/to/WebOptEnv/web_opt/server/Dockerfile`
